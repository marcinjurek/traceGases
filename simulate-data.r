## simulate y given x 
simulate_y <- function(x, avail.obs, lik.params) {
    ...
    data_model <- lik.params["data.model"]
    # simulate data
    if (data_model == "poisson") {
        y.obs <- rpois(n.obs, exp(x[obs.inds]))
    } else if(data_model=="logistic") {
        y.obs <- rbinom(n.obs,1,prob = exp(x[obs.inds])/(1+exp(x[obs.inds])))
    } else if(data_model=="gamma") {
...
## simulate x
simulate_xy <- function(x0, E, Qc, avail.obs, lik.params, Tmax,
                        seed=NULL, sig2=NULL, smooth=NULL, range=NULL,
                        locs = NULL) {
    ...
    if (is.null(Qc)) {
        ...
        Q <- GPvecchia::MaternFun(dist_mat, covparms)
        Qc <- t(chol(Q))
    }
    ...
    for (t in 1:Tmax) {
        ...
        if( class( avail.obs ) == "list" )  {
            y[[t]] <- simulate_y(x[[t]], avail.obs[[ t ]], lik.params)
        } else {
            y[[t]] <- simulate_y(x[[t]], avail.obs, lik.params)
        }
    } 
